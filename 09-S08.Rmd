...
To complete the tasks, download the four Stata files from Blackboard Section 8 learning materials (us_a.dta, us_b.dta, us_c.dta, and us_d.dta). 
...
## 1. Task: Importing Data {-}


 will use only a selection of variables. *Hint: as you go through the practical, you will develop an understanding of what these variables measure. This should help you form some ideas about why they were chosen; provide an explanation in your research notebook about this aspect.*
...
Now let's import the `a_indresp.dta` file that you should have placed in a folder called `data` and store it in an object called `us1`. You already know how to do this, but how about if we only want to import the variables that are stored in `vars1`? To do so, we must add a new argument to the `read_dta()` function called `col_select` and specify the name of the object containing the list of variables. 

**Run the code below to complete this task.**

```{r, warning = FALSE, message = FALSE}
us1 <- read_dta("data/a_indresp.dta",
                     col_select = vars1)
```

**Now describe the data using the `head()` and `glimpse()` commands. \textcolor{violet}{Hint: you may want to explain the difference between these two functions in your notebook.}**


One feature of the Understanding Society variable names is that the survey uses letters as a prefix to denote the wave of data in the variable name. So, for example, the letters "a" and "b" at the start of the variable names correspond to waves 1 and 2 respectively so these will changing later. Also worth noting that "pidp" doesn't have a prefix. This is because this variable stores the ID of the survey respondent, so it will not change over time. 

\newpage

Let's do the same thing with Wave 2 as we did earlier for Wave 1. This time we will want fewer variables. With longitudinal data, we can treat some variables as time constant and others as time varying. Some variables from the first wave will be treated as time constant so they don't need to be measured every wave. These variables are "sex", "age", "highqual" (education). The rest of the variables will need to be imported from every wave as they can change over time. 

**Run the code below to create a new object called `vars2` containing the variables that you want to import from `b_indresp.dta` (Wave 2).**

```{r}
vars2 <- c("pidp", "b_single_dv",
           "b_urban_dv", "b_vote6", "b_fimngrs_dv", "b_sclfsato",
           "b_sf12pcs_dv", "b_sf12mcs_dv", "b_istrtdaty" )
```

**Use the same approach as for Wave 1 to import only the selected variables from Wave 2. Name this object `us2`.**

```{r, echo=FALSE, warning = FALSE, message = FALSE}
us2 <- read_dta("data/b_indresp.dta",
                     col_select = vars2)
```


**Now create two new character vectors, `vars3` and `vars4` so that they contain the same variables as `vars2`. Then import these variable from `c_indresp.dta` (Wave 3) and `c_indresp.dta` (Wave 4).**


```{r, echo = FALSE, warning=FALSE, message=FALSE}
# make object with the variable names we want
vars3 <- c("pidp", "c_single_dv",
           "c_urban_dv", "c_vote6", "c_fimngrs_dv", "c_sclfsato",
           "c_sf12pcs_dv", "c_sf12mcs_dv", "c_istrtdaty" )

vars4 <- c("pidp", "d_single_dv",
           "d_urban_dv", "d_vote6", "d_fimngrs_dv", "d_sclfsato",
           "d_sf12pcs_dv", "d_sf12mcs_dv", "d_istrtdaty" )

us3 <- read_dta("data/c_indresp.dta",
                     col_select = vars3)

us4 <- read_dta("data/d_indresp.dta",
                     col_select = vars4)
```

\newpage

## 1.2 Data Management, Merging, and Reshaping

To analyse data at longitudinal level and therefore identify and investigate trends over time, all waves must be merged together in a single dataframe. However, before any merging is performed, the data in each wave must be adequately prepared. First, we'll rename the variables so that they can be more easily handled in later tasks. 

Let's start with a simple example. 

**Have a look at the names of the `us1` object you have imported and then use the `rename()` command to change the `a_dvage` variable to `a_age`. **
