...
Use the `filter()` function remove the missing values from the variable. 
...
Using the appropriate function, find the number of cases. Are these are missing values?
...
Explore the `ess_country_full` object using `glimpse()`. 
...
Using the appropriate function, subset `ess_country_full` such that it only contains the variable `cntry` and variables from 2018. 
...
Use the appropriate function to check whether the observations for the **cntry** variable in **ess_cntry18** are unique. 
...
Use the `count` function to confirm the changes.  
...
Use the appropriate functions to transform the `imbgeco` variable into a factor variable called `immigration` with three categories as follows: 
...
Use the appropriate functions to create a new factor variable called `immigrants` based on the`imwbcnt` variable such that: 
...
# 9. Saving Data Objects 

Now export the **ess2** data object as a `.csv` file using the `write_csv()` function from the `readr` core tidyverse package. 
