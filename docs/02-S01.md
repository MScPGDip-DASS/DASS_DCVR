...
::: {.r-comment style="display: none;"}
When applying the `class()` function to the two vectors, your R output should have confirmed that **num_vct** is a "numeric" vector and that the **char_vct** is a "character" vector.
:::

## 2.2 Exercise {.unnumbered}

The number of elements in a vector is referred to as its *length*. With small vectors such as those you have already created, you can simply count these. Alternatively, the `length()` function can be used.  
...
Export both **bsas_spss** and **bsas_stata** data objects in `.RData` format. Name this file **bsas_objects**. 
...
Now export **bsas_spss** as a single object in `.RDS` format. Name this file **bsas_spss_file**. 
...
Finally, export the **bsas_stata** object as a `.csv` file. Name this file **bsas_csv_file**. 
