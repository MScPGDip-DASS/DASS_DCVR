...
# 1. Performing Calculations with Vectors {.unnumbered}
...
Perform the following tasks with the `num_vct` object:
...
When performing each of the operations above, you will note in the output that the operation is applied to each individual element within the vector.
:::
...
Perform the following tasks with the following two vectors: `num_vct` and `num_vct2`.
...
::: {.r-comment style="display: none;"}
When performing each of the operations between the two vectors, you will note in the output that the operation is being applied to the element of each vector in sequential order, according to the index of each element.
...
# 2. Exploring Vectors {.unnumbered}
...
::: {.r-comment style="display: none;"}
Your R output when applying the `class()` function to the two vectors should have confirmed that `num_vct` is a "numeric" vector and that the `char_vct` is a "character" vector.
:::
...
::: {.r-comment style="display: none;"}
Applying the function for each one of the vectors should have shown you a single number as an output, namely, the number of elements each of the vectors contains.
:::
...
# 3. Importing and Exporting Data {.unnumbered}
...
You will also have the opportunity to practice how to export data in various formats.
...
Navigate to the <a href="https://beta.ukdataservice.ac.uk/datacatalogue/studies/study?id=8850#!/access-data" target="_blank">British Social Attitudes Survey (2019) webpage</a> on the UK Data Service website.

Scroll down to the end of the webpage and download the dataset in both SPSS and STATA formats on your computer. 

To access the SPSS data file, unzip folder and open the folders in the following sequence:  

*UKDA-8850-spss \> spss \> spss25* 

Follow the same approach for accessing the STATA file.   

Place these two files in your root R project working directory in a subfolder called **data**. 

::: note
The recommended approach for a ‘clean’ working directory is to place all the data files you plan to use in a separate folder within your R project working directory. You should always use simple names that allow you easy access to the contents when you want to either explore the folder on your machine or specify the path to these folders in R. 
:::
...
## 3.5 Exercise {.unnumbered}

Finally, export the `bsas_stata` object as a `.csv` file.


```r
write.csv(bsas_stata, "data/bsas_stata_file.csv")
```

## 3.6 Exercise {.unnumbered}
...
```r
bsas_stata2 <- read.csv("data/bsas_stata_file.csv")
```
...
# 4. Bonus Task: Data on GitHub {.unnumbered}
